...
install_common_dependencies()
{
    ...
    ${SUDO} apt-get -y install libboost-filesystem-dev libboost-system-dev libboost-program-options-dev 
    ${SUDO} apt-get -y install libboost-test-dev libeigen3-dev libode-dev wget libyaml-cpp-dev
    export CXX=g++
    export MAKEFLAGS="-j `nproc`"
    echo "===========Finished common deps"
}

install_python_binding_dependencies()
{
    echo "===========installing python bindings (-dev and -pip)"
    ${SUDO} apt-get -y install python${PYTHONV}-dev python${PYTHONV}-pip
    # install additional python dependencies via pip
    echo "===========installing pygccxml and pypluplus"
    ${SUDO} pip${PYTHONV} install --break-system-packages -vU https://github.com/CastXML/pygccxml/archive/develop.zip pyplusplus
    # install castxml
...
install_app_dependencies()
{
    ...
    # install additional python dependencies via pip
    echo "install PyOpenGL-acelerate"
